...
def callback(image, boxes):
    ...
    # print("image time:", image.header.stamp.to_sec()*1000)
    # print("boxes time:", boxes.header.stamp.to_sec()*1000)
    print("diff time: ", image.header.stamp.to_sec()*1000 - boxes.header.stamp.to_sec()*1000)
    frame_count += 1

    # list_track = tracker.sort_track(boxes)
    ...
    for index in range(0, len(multi_line)):
        ...
        up_count[index], down_count[index] = utils.traffic_count_track(cv_image, boxes.bounding_boxes, classes_list,  polygon_mask_blue_and_yellow, 
                                                                blue_list[index], yellow_list[index],  up_count[index], down_count[index])
    ...
    if show_image:
        ...
        for i in range(0, len(boxes.bounding_boxes)):
            track_id = boxes.bounding_boxes[i].id
            ...
            # 绘制 目标类别
            cv2.putText(cv_image , str(cls), (int(x1), int(y1)+15), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), lineType=cv2.LINE_AA)     
